...
def update_state_space(
        period, choice, lagged_choice, policy_state, retirement_age_id, experience,
        options
):
    next_state = dict()
...
def state_specific_choice_set(period, lagged_choice, policy_state, options):
    age = period + options["start_age"]
    min_individual_retirement_age = options["min_retirement_age"] + policy_state * \
                                    options["belief_update_increment"]
...
def solve_final_period_scalar(
    ...
    value = compute_utility(consumption=begin_of_period_resources, choice=choice,
                            params_dict=params)
...
def budget_constraint(lagged_choice,  # d_{t-1}
    ...
    SRA_at_resolution = options["mininum_SRA"] + policy_state * options[
        "belief_update_increment"]
